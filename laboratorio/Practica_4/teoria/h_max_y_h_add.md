...
Son conocidas como la heurística Max ($h^{\max}$) y la heurística Aditiva ($h^{\text{add}}$).

## ¿qué significa cada término y cómo funciona cada fórmula.?
...
- Caso Base ($0 \text{ si } G = \{g\} \text{ y } g \in s$): Si tu objetivo es una sola meta $g$ y esa meta ya es verdadera en tu estado actual $s$, el coste para alcanzarla es $0$.
- Búsqueda de Acciones ($\min(\dots) \text{ si } G = \{g\} \text{ y } g \notin s$): Si tu objetivo es una sola meta $g$, pero no está en el estado actual, tienes que buscar una acción para conseguirla. Revisas todas las acciones posibles ($a \in A$) que produzcan esa meta ($g \in \text{add}(a)$).
- El coste será el mínimo posible sumando: el coste de la acción en sí ($c_a$) más el coste acumulado de satisfacer las precondiciones de esa acción ($h^{\max}(s, \text{pre}(a))$).
- Metas Múltiples ($\max(\dots) \text{ si } |G| > 1$): Si tienes un conjunto con múltiples metas, la heurística asume que **el coste total** para alcanzar todo el conjunto es simplemente **el coste de la meta más difícil de conseguir** (el valor máximo entre los costes individuales).

_Nota:_ La última línea ($h^{\max}(s) = h^{\max}(s, S_G)$) simplemente indica que el valor heurístico general para un estado $s$ equivale a aplicar esta fórmula al conjunto total de metas finales del problema.

### La Heurística Aditiva: $h^{\text{add}}$
...
- Caso Base: Igual que arriba, cuesta $0$ si ya estás ahí.
- Búsqueda de Acciones: Igual que arriba, busca la acción más barata para lograr una meta individual.
- Metas Múltiples ($\sum \dots \text{ si } |G| > 1$):
...
- $h^{\max}$ es optimista (es una heurística "admisible"). Piensa: "Si tengo que hacer la tarea A que cuesta 3 y la tarea B que cuesta 5, como puedo hacerlas a la vez, el coste total será 5".
- $h^{\text{add}}$ es pesimista y no toma en cuenta que una misma acción podría ayudar a lograr ambas metas (es "inadmisible"). Piensa: "Si tengo que hacer la tarea A que cuesta 3 y la tarea B que cuesta 5, el coste total será 8". Aunque sobreestima el coste, **suele guiar a los algoritmos de búsqueda de forma mucho más rápida en la práctica**.
